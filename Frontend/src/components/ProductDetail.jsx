...
const ProductDetail = () => {
  ...
  const handleAddToCart = async () => {
    if (!token) return toast.info("Please login first", { toastId: "login-cart" });
    if (!selectedSize) return toast.error("Please select a size first", { toastId: "select-size" });
    if (isInCart) return toast.info("Already in cart", { toastId: "already-cart" });
  ...
  const handleAddToWishlist = async () => {
    if (!token) return toast.info("Please login first", { toastId: "login-wishlist" });
    if (isInWishlist) return toast.info("Already wishlisted", { toastId: "already-wishlist" });
    ...
    <div className="backdrop-blur-lg min-h-screen py-12">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 px-4 sm:px-8">
        ...
        <div className="flex flex-col justify-start">
          <p onClick={() => window.history.back()} className="text-sm text-gray-500 mb-2 cursor-pointer hover:underline">
            ← Back to Products
          </p>
          ...
          <ul className="text-sm text-gray-600 mb-5 list-disc list-inside">
            <li>100% Cotton</li>
            <li>Free shipping within 4–6 days</li>
